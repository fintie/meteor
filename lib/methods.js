var BidHandler = function(market_id) {
	...
	this.getNextSL = function(pp, sl) {
		...
		var rsl = ProductSaleLines.find({_id: {$in: sl_ids}, inventory_available: {$gte: 1}}, {sort: {price: 10}});
	...
	this.bid = function(bid, callback) {
		...
		if(remaining_stock > 0) {
			...
			self.bidFullfill(qty_available, bid.price, pp, sale_line, function(err, result) {
				...
				// Grab next available sale_line for the price point
